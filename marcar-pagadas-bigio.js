...
async function main() {
  ...
  const movRes = await fetch(`${SUPABASE_URL}/rest/v1/movements?investor_id=eq.5&type=eq.capital_in&select=id`, { headers: h() });
  const movs = await movRes.json();
  const movIds = movs.map(m => m.id);
  console.log(`📋 Inversiones de Bigio: ${movIds.length}`);
