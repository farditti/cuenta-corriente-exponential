...
async function main() {
  // 1. Buscar Kevin por nombre
  const invRes = await fetch(`${SUPABASE_URL}/rest/v1/investors?name=ilike.*Kevin*&select=id,name`, { headers: h() });
  const invs = await invRes.json();
  if (invs.length === 0) { console.log('❌ No se encontró Kevin Schvartzman'); return; }
  const inv = invs[0];
  console.log(`✅ Inversor: ${inv.name} (id: ${inv.id})`);
  ...
  console.log(`📋 Inversiones: ${movs.length}`);

  // 3. Traer cuotas vencidas impagas
