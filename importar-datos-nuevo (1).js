...
function buildSchedule(id, amount, rate, startDate, endDate, frequency) {
  ...
  // Última cuota proporcional si endDate no coincide con el último vencimiento
  const lastScheduled = schedule.length > 0 ? schedule[schedule.length - 1].due_date : startDate;
  if (lastScheduled < endDate) {
    const lastDay = parseInt(lastScheduled.split("-")[2]);
    const endDay  = parseInt(endDate.split("-")[2]);
    let lastAmt;
    if (lastDay === endDay) {
      const d1 = new Date(lastScheduled + "T12:00:00");
      const d2 = new Date(endDate + "T12:00:00");
      const mElapsed = (d2.getFullYear() - d1.getFullYear()) * 12 + (d2.getMonth() - d1.getMonth());
      lastAmt = parseFloat((amount * rate / 100 / 12 * mElapsed).toFixed(2));
    } else {
      const days = daysBetween(lastScheduled, endDate);
      lastAmt = parseFloat((amount * dailyRate * days).toFixed(2));
    }
    if (lastAmt > 0) {
      schedule.push({ schedule_id: `${id}_${endDate}_f`, capital_mov_id: id, due_date: endDate, amount: lastAmt, partial: true, partial_days: daysBetween(lastScheduled, endDate), paid: false, paid_date: null, snapshot_capital: amount, snapshot_rate: rate });
    }
  }
